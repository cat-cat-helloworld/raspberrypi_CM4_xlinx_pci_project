...
static int engine_status_read(struct xdma_engine *engine, bool clear, bool dump)
{
    ...
    u32	w = read_register(&engine->regs->status);
	pr_info(" ***************************************** %s: ioread32(0x%p) = 0x%08x (status).\n", engine->name,
		&engine->regs->status, w);
...
static struct xdma_transfer *engine_start(struct xdma_engine *engine)
{
	...
	//w = le32_to_cpu(PCI_DMA_L(transfer->desc_bus));
	...
	//w = le32_to_cpu(PCI_DMA_H(transfer->desc_bus));
	...
	dbg_init("****************** engine %s pre-alloc desc: 0x%p, desc_bus:0x%llx.************************\n",
			  engine->name, engine->desc,
			 engine->desc_bus);

    //r = read_register(&engine->sgdma_regs->first_desc_lo);
	pr_info("********************************************* ");

	//r = read_register(&engine->sgdma_regs->first_desc_hi);
	//pr_info("********************************xdma_transfer*********** sgdma_regs->first_desc_hi:%04x \n",r);
...
static int engine_service(struct xdma_engine *engine, int desc_writeback)
{
	...
	int rv = 0;
	pr_info("%s,%d",__func__,__LINE__);
    pr_info("line:%d,desc_count:%d",__LINE__,desc_count);
	if (!engine) {
		pr_err("dma engine NULL\n");
		return -EINVAL;
	}
	...
	dbg_tfr("engine->name %s wb 0x%x, desc_count %u, err %u, dequeued %u.\n",
...
static irqreturn_t xdma_isr(int irq, void *dev_id)
{
	...
	if (mask) {
		...
		int max = xdev->h2c_channel_max;
         dbg_irq("****************************iterate over H2C (PCIe read)");    
	...
	if (mask) {
		...
		int max = xdev->c2h_channel_max;
         dbg_irq("****************************iterate over C2H (PCIe write)");    
...
static void xdma_desc_adjacent(struct xdma_desc *desc, u32 next_adjacent)
{
	...
	control |= DESC_MAGIC | (next_adjacent << 8);
...
static int engine_alloc_resource(struct xdma_engine *engine)
{
    ...
    of_dma_configure(&xdev->pdev->dev, xdev->pdev->dev.of_node,true);
		       ...
		       int offset, enum dma_data_direction dir, int channel)
{
	...
	if (rv){
	pr_info("engine_alloc_resource failed,%d ,line:%d",rv,__LINE__);
		return rv;
	}
	...
	if (rv){
		pr_info("engine_init_regs failed,%d,line:%d",rv,__LINE__);
		return rv;
	}
			...
			struct xdma_request_cb *req, struct xdma_transfer *xfer)
{
	...
	dbg_init("********transfer_init********** engine %s pre-alloc desc: 0x%p, desc_bus:0x%llx.************************\n",
			  engine->name, engine->desc,
			 engine->desc_bus);
...
pr_info("%s,%d",__func__,__LINE__);
	...
	while (nents) {
		...
		spin_lock_irqsave(&engine->lock, flags);
        pr_info("********************************start the engine*********** xfer->state:%d \n",xfer->state);
		switch (xfer->state) {
		case TRANSFER_STATE_COMPLETED:
			spin_unlock_irqrestore(&engine->lock, flags);
				pr_info("******TRANSFER_STATE_COMPLETED******\n");
			rv = 0;
		...
		case TRANSFER_STATE_FAILED:
			...
			pr_info("TRANSFER_STATE_FAILED xfer 0x%p,%u, failed, ep 0x%llx.\n", xfer,
...
pr_info("%s,%d",__func__,__LINE__);
	...
	nents = req->sw_desc_cnt;
	while (nents) {
			r = read_register(&engine->regs->completed_desc_count);
	       pr_info("********************************start the engine*********** regs->completed_desc_count:%04x \n",r);
...
int xdma_performance_submit(struct xdma_dev *xdev, struct xdma_engine *engine)
{
	...
	dbg_init("******************device %s, engine %s pre-alloc desc: 0x%p, desc_bus:0x%llx.************************\n",
			 dev_name(&xdev->pdev->dev), engine->name, engine->desc,
			 engine->desc_bus);
