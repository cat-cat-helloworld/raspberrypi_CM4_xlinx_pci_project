		...
		loff_t *pos)
{
	...
	if (rv){
		...
		printk("Copy to userspace failed but continuing\n");
	}
			...
			size_t count, loff_t *pos)
{
	...
	/* only 32-bit aligned and 32-bit multiples */
	if (*pos & 3){
		printk("char_ctrl_write error 89\n");
		return -EPROTO;
	}
	...
	if (rv)
		...
		{
			printk("copy from user failed %d/4, but continuing.\n", rv);
		}
...
static long version_ioctl(struct xdma_cdev *xcdev, void __user *arg)
{
	...
	if (rv) {
		...
		// 	rv, sizeof(struct xdma_ioc_info));
				printk("copy from user failed %d/%ld.\n",
			rv, sizeof(struct xdma_ioc_info));	
		return -EFAULT;
	}
